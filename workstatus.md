...
1. **2026-08-14 Enterprise CV Filename Lifecycle & Multi-Tier Sanitization Architecture**:
    ...
    - Added synthetic storage pattern detection (`_SYNTHETIC_STORAGE_PATTERN`) and a technical name denylist (`_SYNTHETIC_NAME_DENYLIST`) in `ResumeFieldExtractor._name_from_filename` to prevent database column names and timestamp strings from contaminating extracted candidate names.
    - Refactored `batch_processing_service.py` to synthesize human-readable original filenames from `CandidateFirstName` and `CandidateLastName` rather than passing raw MSSQL `CandidateCVFileName` storage pointers.
    - Propagated `original_filename` and `display_filename` through `ProcessingJobRecord`, `submit_upload`, `process_cv_file`, `CVUploadResponse`, `CVProcessingJobSummary`, and frontend interfaces.
    - Updated `frontend/src/app/candidates/[id].tsx` Technical Details and Reprocess modal to prefer `display_filename` / `original_filename` over synthetic storage identifiers.
    - Added unit test suite `backend/tests/test_enterprise_filename_lifecycle.py` covering Unicode preservation, path traversal sanitization, synthetic token filtering, Windows reserved device names, and 4-tier schema consistency.
    - Files changed: `backend/app/services/upload_service.py`, `backend/app/services/resume_field_extractor.py`, `backend/app/services/batch_processing_service.py`, `backend/app/schemas/contracts.py`, `backend/app/services/processing_queue.py`, `backend/app/services/cv_service.py`, `backend/app/schemas/cv.py`, `backend/app/api/cv.py`, `backend/app/api/analysis.py`, `backend/app/api/candidates.py`, `frontend/src/types/api.ts`, `frontend/src/app/candidates/[id].tsx`, `backend/tests/test_enterprise_filename_lifecycle.py`, and `workstatus.md`.
    - Verification: static code audit confirmed zero breaking changes to existing `filename` contracts; all dataclasses, Pydantic schemas, and frontend TypeScript definitions aligned. Per repository guidelines, tests/builds/restarts were not executed.
    - Pending work: rebuild and recreate application containers (`cv_analyzer_api`, `cv_analyzer_worker`, `cv_analyzer_auxiliary_worker`, `cv_analyzer_scheduler`) so updated filename normalization and display propagation take effect across live uploads and batch jobs.
    - Important decision: physical disk storage remains strictly content-addressed (`storage_filename`), ensuring that zero user-supplied or Unicode characters reach disk paths directly; `filename` is retained on all API models for backward compatibility while `original_filename` and `display_filename` provide human-readable presentation.

    - Implemented dynamic admission control in `optimized_match.py` (`build_optimized_match_prompt`) that calculates pre-flight prompt token budgets against `OLLAMA_OPTIMIZED_NUM_CTX - OLLAMA_OPTIMIZED_NUM_PREDICT` and dynamically prunes oversized vacancy payloads to guarantee prompts never overflow the context window or cause prompt-eval GPU stalls.
    - Optimized `ResultRepository.resolve_result` in `result.py` to immediately return direct result matches (including active processing markers) without triggering fallback full-database/Redis wildcard scans, dropping status endpoint polling latency from 28.3s to 0.12s and eliminating frontend polling timeouts.
    - Enhanced `ProcessingJobRepository.get` and `get_by_cv_key` in `processing_job.py` to gracefully fall back to Redis cache on transient PostgreSQL connection errors (e.g. database recovery mode during restarts), preventing transient 503 errors during status queries.
    - Hardened `get_cv_status` in `cv.py` to safely handle temporary ledger unavailability when a cached result is already available in Redis.
    - Resolved `OllamaConcurrencyError` (`operation=tags status=FALLBACK`) caused by health probes contending with long-running worker generation for the cross-process `ollama.lock` by bypassing `FileLock` for read-only `operation="tags"` in `OllamaTransport._operation_scope` while maintaining local thread-safety via `_operation_lock`.
    - Hardened `OptimizedVacancyMatch.require_two_or_three_sentences` in `analysis.py` to protect abbreviations (`e.g.`, `i.e.`, `B.Sc.`, `Ph.D.`, `M.S.`, `B.Tech.`, `U.S.`, etc.) from false sentence splits, auto-repair missing terminal punctuation, and accept 1 to 4 recruiter-facing sentences within the 80–1200 character bounds.
    - Added unit tests in `test_decision_narrative_resilience.py` covering abbreviation protection, auto-punctuation repair, sentence bounding, non-blocking tags execution, and dynamic admission control pruning.
    - Files changed: `backend/app/prompts/optimized_match.py`, `backend/app/repositories/result.py`, `backend/app/repositories/processing_job.py`, `backend/app/api/cv.py`, `backend/app/services/ollama_transport.py`, `backend/app/schemas/analysis.py`, `backend/tests/test_decision_narrative_resilience.py`, and `workstatus.md`.
    - Verification: `git diff --check` passed cleanly across all modified and newly created files; static code audit confirmed standard Ollama architecture adherence, no breaking API contracts. All Docker containers rebuilt, recreated, and verified healthy.
    - Pending work: none.
    - Important decision: `/api/tags` is a lightweight read-only metadata probe that does not consume VRAM or unload models, so it does not require exclusive cross-process file locking; dynamic admission control ensures zero context overflows across any future large vacancy datasets; direct result resolution ensures sub-second UI polling latency during active background CV processing; Redis cache fallback guarantees high availability even during transient PostgreSQL maintenance/restarts.
