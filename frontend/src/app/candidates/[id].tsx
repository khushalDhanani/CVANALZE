...
export default function CandidateDetailScreen() {
    ...
    try {
      ...
      pollTimerRef.current = setInterval(async () => {
        ...
        if (pollCount >= 40) {
          stopTimers();
          setIsReprocessing(false);
          setReprocessError('Reprocessing timed out (TIMED_OUT). Please try again.');
          return;
        }
  ...
  const renderOverviewTab = () => (
    <View className="flex-row flex-wrap gap-4 md:flex-nowrap">
      ...
      <View className="w-full md:w-[45%] lg:w-5/12 gap-4">
        ...
        <Card className="gap-2 p-3 shadow-none border-border">
          ...
          <View className="flex-row items-center gap-2">
            ...
            <Text className="font-sans text-sm text-text-primary">{data?.email || "—"}</Text>
          </View>
          <View className="flex-row items-center gap-2">
            <Phone size={14} color={COLORS.textMuted} />
            <Text className="font-sans text-sm text-text-primary">{data?.phone || "—"}</Text>
          </View>
          <View className="flex-row items-center gap-2">
        ...
        {((data?.work_experience && data.work_experience.length > 0) ||
          ...
          (data?.normalized_resume?.employment && data.normalized_resume.employment.length > 0)) && (
            <Card className="gap-3 p-3 shadow-none border-border">
              <Text className="text-xs tracking-wider uppercase font-sans-bold text-text-muted">Experience</Text>
              {(
  ...
  const renderTimelineTab = () => (
    <View className="gap-4">
      ...
      {reprocessError && (
        <Card className="flex-row items-center justify-between p-3 bg-danger/10 border-danger/30">
          <View className="flex-row items-center flex-1 gap-2 pr-2">
            <AlertCircle size={14} color={COLORS.danger} />
            <Text className="flex-1 text-xs font-sans-medium text-danger">{reprocessError}</Text>
          </View>
      ...
      <Card className="p-3 shadow-none border-border">
        ...
        <View className="gap-2">
          ...
          <View className="flex-row items-center justify-between">
            ...
            <Text className="font-mono text-xs text-text-primary">{data?.page_count || 1}</Text>
          </View>
          <View className="flex-row items-center justify-between">
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F8FAFC]">
      ...
      <View className="z-10 px-4 py-3 bg-white border-b shadow-sm border-border">
        <View className="flex-row items-start justify-between">
          ...
          <View className="flex-row items-center gap-3">
            ...
            {data?.experience_years != null ? (
              <View className="items-center px-3 py-1 border rounded bg-background border-border">
            ...
            <View className="flex-row gap-2">
              <View className="h-8 overflow-hidden rounded-md bg-secondary">
                <Button
                  variant="secondary"
                  size="sm"
              ...
              <View className="w-8 h-8 overflow-hidden border rounded-md bg-secondary border-border">
                <Button
                  variant="ghost"
                  size="sm"
      ...
      {/* 2. Tab Navigation */}
      <View className="flex-row gap-4 px-4 overflow-x-auto bg-white border-b border-border">
        {[
