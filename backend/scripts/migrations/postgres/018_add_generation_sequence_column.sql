...
ALTER TABLE cvai.cv_results ADD COLUMN IF NOT EXISTS generation_sequence BIGINT DEFAULT nextval('cv_results_generation_seq');

-- 3. Safely backfill existing null rows
UPDATE cvai.cv_results 
...
ALTER TABLE cvai.cv_results ALTER COLUMN generation_sequence SET DEFAULT nextval('cv_results_generation_seq');

-- 5. Create index on generation_sequence
CREATE INDEX IF NOT EXISTS idx_cv_results_generation_sequence ON cvai.cv_results(generation_sequence);
