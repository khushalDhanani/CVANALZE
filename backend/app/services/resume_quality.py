...
class ResumeQualityMetrics:
    ...
    def compute(
        ...
        sections_detected = [
            name
            for name, pattern in RuleConfigManager.get_compiled_section_patterns().items()
        ...
        has_location = bool(
            location
            and location_confidence >= resume_quality.location_acceptance_min_confidence
        )
        contact_weights = resume_quality.contact_weights
        contact_score = (
            (contact_weights.get("email", 0.10) if has_email else 0.0)
            + (contact_weights.get("phone", 0.10) if has_phone else 0.0)
            + (contact_weights.get("location", 0.05) if has_location else 0.0)
        )
