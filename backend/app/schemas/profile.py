...
class CareerTransition(BaseModel):
    ...
    reason_inferred: str = Field(
        ..., description="Inferred reason for the transition based on the CV."
    )
...
class DynamicCandidateProfile(BaseModel):
    education_domains: list[str] = Field(
        default_factory=list, description="Domains of education (e.g. Computer Science, Accounting)."
    )
    professional_domains: list[str] = Field(
        default_factory=list, description="Domains of professional experience (e.g. Software Engineering, Sales)."
    )
    ...
    career_transitions: list[CareerTransition] = Field(
        default_factory=list, description="Any detected significant career or domain transitions."
    )
    core_skills: list[str] = Field(
        default_factory=list, description="Core technical and soft skills demonstrated in the CV."
    )
    ...
    timeline: list[TimelineEvent] = Field(
        default_factory=list, description="Chronological timeline of education and experience."
    )
