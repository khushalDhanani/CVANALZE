...
from app.services.match_evaluators import _has_relevant_experience
import fitz
...
for cv in cvs:
    ...
    matched_res, _ = ScoringEngine._extract_term_matches(context.domain_candidate_text or context.norm_text, job_ctx.responsibilities)
    
    has_rel = _has_relevant_experience(context, job_ctx, matched_res)
    print(f"Has Relevant Experience boolean: {has_rel}")
