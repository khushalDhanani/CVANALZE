...
def test_optimized_prompt_cache_key_includes_required_version(monkeypatch):
    ...
    monkeypatch.setattr(settings, "OPTIMIZED_PROMPT_VERSION", "3.8")
    ...
    assert ":3.8:" in cache_get.call_args.args[0]
...
def test_required_optimized_match_prompt_validates_version_placeholders_and_schema(db_session, monkeypatch):
    ...
    prompt = PromptTemplateMaster(
        ...
        version_tag="3.8",
        system_instruction="VACANCY COVERAGE: evaluate every supplied vacancy. DECISION NARRATIVES: return three detailed sections. {input_json} {domain_list_str} {dept_list_str}",
    ...
    db_session.commit()
    assert PromptService.check_required_optimized_match_prompt().ready is False
    prompt.system_instruction = "/think\nVACANCY COVERAGE: evaluate every supplied vacancy. DECISION NARRATIVES: return three detailed sections. {input_json} {domain_list_str} {dept_list_str}"
    db_session.commit()
    assert PromptService.check_required_optimized_match_prompt().ready is False
    prompt.system_instruction = "VACANCY COVERAGE: evaluate every supplied vacancy. DECISION NARRATIVES: return three detailed sections. {input_json} {domain_list_str} {dept_list_str}"
