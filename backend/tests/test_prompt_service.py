...
def test_optimized_prompt_cache_key_includes_required_version(monkeypatch):
    ...
    monkeypatch.setattr(settings, "OPTIMIZED_PROMPT_VERSION", "3.9")
    ...
    assert ":3.9:" in cache_get.call_args.args[0]
...
def test_required_optimized_match_prompt_validates_version_placeholders_and_schema(db_session, monkeypatch):
    ...
    prompt = PromptTemplateMaster(
        ...
        version_tag="3.9",
        system_instruction="VACANCY COVERAGE: evaluate every supplied vacancy. DECISION NARRATIVES: return three detailed sections. REQUIREMENT ASSESSMENTS: return every requirement. {input_json} {domain_list_str} {dept_list_str}",
    ...
    prompt_schema = json.loads(prompt.expected_schema_json)
    prompt_schema["properties"]["matched_vacancies"]["items"]["properties"]["requirement_assessments"] = {
        "type": "array",
        "items": {
            "type": "object",
            "required": sorted(PromptService.OPTIMIZED_MATCH_ASSESSMENT_SCHEMA_FIELDS),
            "properties": {field: {} for field in PromptService.OPTIMIZED_MATCH_ASSESSMENT_SCHEMA_FIELDS},
        },
    }
    prompt.expected_schema_json = json.dumps(prompt_schema)
    db_session.add(prompt)
    db_session.commit()
    ...
    db_session.commit()
    assert PromptService.check_required_optimized_match_prompt().ready is False
    prompt.system_instruction = "/think\nVACANCY COVERAGE: evaluate every supplied vacancy. DECISION NARRATIVES: return three detailed sections. REQUIREMENT ASSESSMENTS: return every requirement. {input_json} {domain_list_str} {dept_list_str}"
    db_session.commit()
    assert PromptService.check_required_optimized_match_prompt().ready is False
    prompt.system_instruction = "VACANCY COVERAGE: evaluate every supplied vacancy. DECISION NARRATIVES: return three detailed sections. REQUIREMENT ASSESSMENTS: return every requirement. {input_json} {domain_list_str} {dept_list_str}"
