...
def test_vacancy_content_change_triggers_reembedding():
    ...
    with patch("app.services.embedding_service.EmbeddingService._call_ollama_batch_embed", return_value=[[0.3] * 768]) as mock_batch:
        JobRepository._cache_vacancy_embeddings(job_v1)
        assert mock_batch.call_count == 1

    with patch("app.services.embedding_service.EmbeddingService._call_ollama_batch_embed", return_value=[[0.4] * 768]) as mock_batch_v2:
