...
def test_extraction_timeout_is_persisted_without_retry(monkeypatch, tmp_path):
    content = b"phase-4-cv"
    submission = ProcessingQueueService.submit_upload(**_submission_kwargs(content))
    source = StoredUpload(
        safe_filename="phase_4.pdf",
    ...
    async def fail_extraction(**_kwargs):
        raise DocumentExtractionTimeoutError("extraction deadline exceeded")

    monkeypatch.setattr(processing_queue, "_process_source", fail_extraction)
    result = process_cv_job(submission.record.job_id)

    failed = ProcessingJobRepository.get(submission.record.job_id)
    assert result["error_code"] == "JOB_STUCK"
    assert failed is not None
    assert failed.state == JobState.FAILED
    assert failed.attempt == 1
    assert failed.error is not None
    assert failed.error.code.value == "JOB_STUCK"
    assert failed.error.retryable is False
