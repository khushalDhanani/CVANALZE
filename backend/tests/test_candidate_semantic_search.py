...
def test_candidate_semantic_query_vector_ranking():
    mock_results = [
        {
            ...
            "match_analysis": {"primary_department": "Engineering", "best_match": {"job_title": "Python Engineer", "score": 92.0}},
            "quality_metrics": {"experience_years": 6.0},
            "resume_json": {"contact_info": {"name": "John Doe"}, "skills": ["Python", "FastAPI", "PostgreSQL"]},
        },
        {
            "id": "cand_2",
            "filename": "jane_smith.pdf",
            "markdown": "# Jane Smith\n\nFrontend React UI Specialist with 4 years experience in CSS, Tailwind, TypeScript.",
            "match_analysis": {"primary_department": "UI", "best_match": {"job_title": "React Developer", "score": 88.0}},
            "quality_metrics": {"experience_years": 4.0},
            "resume_json": {"contact_info": {"name": "Jane Smith"}, "skills": ["React", "CSS", "Tailwind"]},
        },
    ]
    ...
    with patch("app.repositories.result.ResultRepository.list_all_results", return_value=mock_results):
        with patch("app.services.embedding_service.EmbeddingService.generate_embedding", return_value=mock_query_emb):
            with patch("app.services.candidate_search_service.CandidateSearchService._vector_search_pg", side_effect=mock_pg_vector):
...
def test_combined_vector_and_structured_filters():
    ...
    with patch("app.repositories.result.ResultRepository.list_all_results", return_value=mock_results):
        with patch("app.services.embedding_service.EmbeddingService.generate_embedding", return_value=[0.1] * 768):
            with patch("app.services.candidate_search_service.CandidateSearchService._vector_search_pg", return_value={"cand_10": 0.88, "cand_20": 0.85}):
