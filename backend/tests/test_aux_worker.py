...
def test_auxiliary_worker_never_subscribes_to_cv_processing_queue():
    ...
    with (
        ...
        patch.object(start_aux_worker, "build_rq_worker_name", return_value="auxiliary-worker-unique") as worker_name,
    ...
    worker_name.assert_called_once_with("cv-analyzer-auxiliary-worker")
    assert worker_class.call_args.kwargs["name"] == "auxiliary-worker-unique"
